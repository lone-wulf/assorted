# contains snippets borrowed from the web
...
def WarpProcessing(inIm, inArr, 
	...
	for i in range(xmini, xmaxi):
		for j in range(ymini, ymaxi):
			...
			if i < 0 or i+1 >= outArr.shape[1]: continue
			if j < 0 or j+1 >= outArr.shape[0]: continue
			...
			if outImgCoord[0] < 0 or outImgCoord[0]+1 >= inArr.shape[1]:
				for chan in range(px.shape[0]): outArr[j,i,chan] = 250
				continue
			if outImgCoord[1] < 0 or outImgCoord[1]+1 >= inArr.shape[0]:
				for chan in range(px.shape[0]): outArr[j,i,chan] = 250
				continue
...
if __name__ == "__main__":
	#Load source image
	srcIm = Image.open("pepsi.jpg")

	im = np.array(srcIm);
	rows = im.shape[0]
	cols = im.shape[1]
	#Create destination image
	dstIm = Image.new(srcIm.mode,(1200,1200))


	#Define control points for warp
	srcCloud = [(1,1),(cols,1),(cols, rows),(1,rows)]
	dstCloud = [(955,198),(1071,506),(847,504),(717,281)]

	#Perform transform
	PiecewiseAffineTransform(srcIm, srcCloud, dstIm, dstCloud)
	
	#result = Image.fromarray((dstIm).astype(numpy.uint8))
	dstIm.save('out.bmp')
	#Visualise result
	#dstIm.show()
